...
# 特别注意：文本分词依赖版本
# 为确保中文文本和合成语音严格对齐，请使用：
#   tiktoken==0.6.0
#   regex==2023.12.25
#
# 其他版本（更高或更低）可能导致 GLM 分词器崩溃，或者在不报错的情况下改变分词结果，从而出现
# “输入一句话，模型读成完全不相干内容（乱读）” 的现象。
...
- `outputs/pretrain_test/example_zh/0.wav` ~ `9.wav`

你可以使用仓库中的 ASR 校验脚本，快速检查“合成语音是否与输入文本一致”：

```bash
python asr_verify_example_zh.py
```

该脚本会：

- 自动加载 Whisper `small` 模型
- 对 `outputs/pretrain_test/example_zh/*.wav` 进行转写
- 将对齐结果写入：
  - `outputs/pretrain_test/example_zh/asr_compare.jsonl`

`asr_compare.jsonl` 每一行包含：

- `uttid`：样本编号
- `target_text`：目标文本（jsonl 中的 `syn_text`）
- `asr_text`：Whisper 转写结果
- `exact_match`：简单归一化后是否完全一致
- `char_overlap`：粗略的字符重合度（0~1）

如果大多数句子的 `char_overlap` 较高（例如 >0.8），且部分样本 `exact_match=true`，说明文本分词与推理链路正常工作；如果全部样本重合度都接近 0，则很可能是分词依赖版本或本地改动导致文本信息丢失，应优先检查：

- 当前环境的 `tiktoken` / `regex` 版本是否与 `requirements.txt` 一致
- 是否修改了 `glmtts_inference.py`、`cosyvoice/cli/frontend.py` 或 `ckpt/vq32k-phoneme-tokenizer/tokenization_chatglm.py`
